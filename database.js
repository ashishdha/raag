...
window.transliterations = new Map();

// ---------- RAAG METADATA ----------
window.tetrachords = new Map();
window.thaats = new Map();
window.jaatis = new Map();
...
window.displayDataLoaded = false;
window.musicDataLoaded = false;
...
window.getScaleTableRow = async function (rowID) {
	...
	console.log("Running getScaleTableRow():", rowID, row);
	return row;
}/**/
...
window.loadDisplayData = async function () 		//This is enough for • notation conversion • language transliterations • pitch calculations
{
	if (displayDataLoaded) { return; }				

	//load all the notation & language helper tables
	await cacheTable("pitchclasses", pitchclasses);
	await cacheTable("octaves", octaves);
	await cacheTable("symbols", symbols);
	await cacheTable("transliterations", transliterations);
	
	buildNotationMap();							//Build the notation lookup map

	displayDataLoaded = true;							//set flag
};

//CACHE RAAG-SPECIFIC HELPER TABLES 			
window.loadMusicData = async function () 	//intentionally separate from the music engine because many pages never need them
{
	if (musicDataLoaded) { return; }			//safety: if it's already loaded, end function
	...
	await cacheTable("confidence", confidence);
	await cacheTable("jaatis", jaatis);
	await cacheTable("tetrachords", tetrachords);
	await cacheTable("thaats", thaats);

	musicDataLoaded = true;					//set flag
};
...
window.string2intArray = function (givenString) {
	...
	if (trimmedString === "") {
		console.log("!!!string2intArray: trimmedString after trim: |" + trimmedString + "| with length:" + trimmedString.length);
		return [];			//EMPTY STRING RETURNS EMPTY ARRAY 
	}
	
	const stringArray = trimmedString.split(/\s+/);	
	//console.log("2 string2intArray: stringArray after split:" + stringArray + " with length:" + stringArray.length);
...
//ARRAY OF INT TO SINGLE NOTATION STRING  
window.intArray2string = function (givenIntArray) {
	if (givenIntArray.length === 0) {
		console.log("!!!intArray2string: givenIntArray is empty, returning blank string!");
		return "";			//return blank string if array is empty
	}	
	//console.log("!!!intArray2string: givenIntArray is VALID");
